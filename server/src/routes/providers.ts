...
import { storeCredentials, getCredentials } from '../services/terraform';
...
providersRouter.post('/:type/accounts', async (req: Request, res: Response) => {
  ...
  if (providerType === 'digitalocean') {
    if (
      !body.credentials ||
      body.credentials.type !== 'digitalocean' ||
      !body.credentials.api_token
    ) {
      throw new AppError(
        400,
        'VALIDATION_ERROR',
        'Invalid credentials for DigitalOcean - api_token required'
      );
    }
...
providersRouter.put('/accounts/:id', async (req: Request, res: Response) => {
  ...
  const updatedAccount = database.getProviderAccount(id);

  if (!updatedAccount) {
    throw new AppError(500, 'UPDATE_FAILED', 'Failed to retrieve updated account');
  }
...
providersRouter.delete('/accounts/:id', (req: Request, res: Response) => {
  ...
  database.deleteProviderAccount(id);
