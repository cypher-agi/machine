...
import type { AuditEvent, AuditEventListFilter, ApiResponse } from '@machina/shared';
...
auditRouter.get('/events', (req: Request, res: Response) => {
  const filters = {
    ...(req.query.action && { action: req.query.action as AuditEventListFilter['action'] }),
    ...(req.query.outcome && { outcome: req.query.outcome as AuditEventListFilter['outcome'] }),
    ...(req.query.actor_id && { actor_id: req.query.actor_id as string }),
    ...(req.query.target_type && { target_type: req.query.target_type as string }),
    ...(req.query.target_id && { target_id: req.query.target_id as string }),
    ...(req.query.after && { after: req.query.after as string }),
    ...(req.query.before && { before: req.query.before as string }),
  } as AuditEventListFilter;
